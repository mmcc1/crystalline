/* Crystalline 2 Symmetric Cipher
 * Copyright: Mark McCarron (Marx Bitware) - 2021
 * 
...
namespace CrystallineCipherLib
{
    ...
    /// </summary>
    public static class Crystalline2
    {
        ...
        private static byte[] CrystallineEncrypt(byte[] data, byte[] key, byte[] salt, byte[] salt2, int rounds)
        {
            while (rounds >= 0)
            {
                ...
                for (int i = 0; i < cipherData.Length; i++)
                {
                    ...
                    PerformByteShiftMod(ref cipherData, i, Shift(key[keyIndex++], salt[saltIndex++], salt2[salt2Index++]), isUp);
