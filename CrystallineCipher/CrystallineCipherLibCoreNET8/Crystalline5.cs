/* Crystalline 5 Symmetric Cipher
 * Copyright: Mark McCarron (Marx Bitware) - 2024
 * 
...
namespace CrystallineCipherLib
{
    ...
    /// Employs a byte shift, byte reversal, bit shift, bit reversal, byte shift pattern
    /// </summary>
    public static class Crystalline5
    {
        ...
        private static byte[] FlipBit(byte[] data)
        {
            BitArray bitArray = new BitArray(data);
            bool[] bools = new bool[bitArray.Length];
            bitArray.CopyTo(bools, 0);

            Array.Reverse(bools, 0, bools.Length);
            BitArray revBitArray = new BitArray(bools);
